...
def pytest_generate_tests(metafunc):
    ...
    parametrize_fixture_with_items(metafunc, "create_json_files", "conf_dict")
    parametrize_fixture_with_items(metafunc, "create_timing_json_files", "conf_dict")
...
@pytest.fixture(scope="module")
def create_timing_json_files(request, tmp_path_factory):
    """Run the confgen to produce the configuration json files
    ...
    timing_conf_dict=request.param
    ...
    if use_connectivity_service:
        timing_conf_dict["boot"]["use_connectivity_service"] = True
        timing_conf_dict["boot"]["start_connectivity_service"] = True

    write_config(configfile, timing_conf_dict)
    ...
    result=CreateJsonResult()
    result.confgen_name=script_name
    result.confgen_config=timing_conf_dict
    result.json_dir=json_dir
    result.log_file=logfile
...
@pytest.fixture(scope="module")
def run_nanotimingrc(request, create_timing_json_files, tmp_path_factory):
    """Run nanorc with the json files created by `create_json_files`. The
    commands specified by the `nanorc_command_list` variable in the
    test module are executed. If `nanorc_command_list`'s items are
    themselves lists, then nanorc will be run multiple times, once for
    ...
    nanorc=request.config.getoption("--nanotimingrc-path")
    if nanorc is None:
        nanorc="nanotimingrc"
    nanorc_options=request.config.getoption("--nanotimingrc-option")
    nanorc_option_strings = []
    if nanorc_options is not None:
        for opt in nanorc_options:
            if len(opt) > 2:
    ...
    run_dir=tmp_path_factory.mktemp("run")
    session_name = "integ-test-timing-session"

    try:
        for config_section in create_timing_json_files.confgen_config.keys():
            if config_section == "timing":
                if "timing_session_name" in create_timing_json_files.confgen_config[config_section].keys():
                    session_name = create_timing_json_files.confgen_config[config_section]["timing_session_name"]
    except ValueError:
        # nothing to do since we've already assigned a default value
        pass
    ...
    timing_session = subprocess.Popen([nanorc] + nanorc_option_strings + [str(create_timing_json_files.json_dir)] + [session_name], cwd=run_dir)
    print(timing_session.communicate(input="boot"))
    print(timing_session.communicate(input="conf"))
    print(timing_session.communicate(input="start"))

    # First run stops here
    yield result

    # This is continued once the other fixtures are complete
    print(timing_session.communicate(input="stop"))
    print(timing_session.communicate(input="scrap"))
    print(timing_session.communicate(input="terminate"))
    print(timing_session.communicate(input="exit"))
    timing_session.wait()

    # result instance should be mutable, so this should update what was yielded
    result.completed_process=timing_session
    result.confgen_name=create_json_files.confgen_name
    result.confgen_config=create_json_files.confgen_config
    result.nanorc_commands=command_list
    result.run_dir=run_dir
...
@pytest.fixture(scope="module")
def par_timing_and_daq(run_nanotimingrc, run_nanorc):
    class MetaResult():
        pass
    
    MetaResult.timing = run_nanotimingrc
    MetaResult.daq = run_nanorc
